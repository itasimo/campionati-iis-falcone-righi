...
function Settings() {
  const [settings, setSettings] = useState(
    () =>
      ...
      JSON.parse(localStorage.getItem("defSettings")),
  );

  useEffect(() => {
    const updateSettings = () => {
      setSettings(JSON.parse(sessionStorage.getItem("settings")));
      document.getElementById("saveTimer").value =
        JSON.parse(sessionStorage.getItem("settings")).saveTimer * 0.001;
    };
  ...
  const handleDeleteClasse = () => {
    OpenPopUp(<EliminaClassePopUp />, 300, null, [
      ["Ok", true],
      ["Annulla", false],
    ]).then((result) => {
      if (result) {
        ...
        getDoc(userRef)
          .then((doc) => {
            if (doc.exists()) {
              ...
              updateDoc(userRef, {
                ...
                console.error(
                  "Errore nella rimozione dell'ID della classe dal documento dell'utente: ",
                  error,
                );
              });
            } else {
          ...
          .then(() => {
            ...
            deleteDoc(doc(db, `Users/${uid}/Classi`, classe.id.toString()))
              ...
              .catch((error) => {
                console.error(
                  "Errore nell'eliminazione del documento della classe dal documento dell'utente: ",
                  error,
                );
              });
          })
          .catch((error) => {
            console.error(
              "Errore nel recupero del documento dell'utente:",
              error,
            );
          });
      }
  ...
  return (
    <div className="border-2 border-secondary rounded-lg p-5 h-1/2 flex flex-col">
      ...
      <div className="ml-5 flex flex-col gap-2 flex-1">
        <div className="flex flex-row gap-1">
          ...
          <input
            ...
            className="w-10"
            onChange={(e) =>
              setSettings({ ...settings, saveTimer: e.target.value * 1000 })
            }
          />
          <p>secondi</p>
        </div>
